...
import { ArrowLeft, ExternalLink, Clock, User, TrendingUp, TrendingDown, Play, MessageCircle, FileText, Send, Bot, Loader2 } from "lucide-react";
...
const ContentDetail = () => {
    ...
    const fetchContent = async () => {
      ...
      try {
        const {
          data,
          error
        } = await supabase.from('content_items').select('*').eq('id', id).single();
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="px-8 py-6 border-b border-border/40">
        <Button variant="ghost" onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-4">
        ...
        <div className="max-w-4xl">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            {content.original_url && <>
                <span>•</span>
                <a href={content.original_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-foreground transition-colors">
          ...
          <div className="flex items-center gap-3 mt-4">
            {content.sentiment && <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getSentimentColor(content.sentiment)}`}>
            ...
            {content.tags && content.tags.length > 0 && <div className="flex flex-wrap gap-2">
                {content.tags.slice(0, 4).map((tag, index) => <span key={index} className="px-2 py-1 bg-muted text-muted-foreground rounded-full text-xs">
                    {tag}
                  </span>)}
              </div>}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        {/* AI Chat Messages */}
        {chatMessages.length > 0 && <div className="space-y-6 mb-8">
            <h2 className="text-lg font-medium text-foreground">AI Insights</h2>
            {chatMessages.map((message, index) => <div key={message.id} className="space-y-4">
                {message.role === 'user' && <div className="flex items-start gap-3">
                ...
                {message.role === 'assistant' && <div className="flex items-start gap-3">
            ...
            {isAILoading && <div className="flex items-start gap-3">
                <Bot className="h-5 w-5 text-blue-600 flex-shrink-0 mt-1" />
      ...
      <div className="fixed bottom-0 left-0 right-0 backdrop-blur-sm border-t border-border/40 p-4 py-[35px] my-0 px-0 mx-[130px] bg-white/95 rounded-none">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3">
            <div className="flex-1 relative">
              <Input placeholder="Ask AI about this content..." value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={handleKeyPress} disabled={isAILoading} className="pr-12 border-border/60 focus:border-primary/50 bg-background h-12" />
              <Button onClick={() => handleAskAI(chatInput)} disabled={!chatInput.trim() || isAILoading} size="sm" className="absolute right-1 top-1 h-8 w-8 p-0">
